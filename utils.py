...
def downloadVideosFromServer(session_id, trial_id, isDocker=True,
    ...
    """
    print()  # whitespace
    print("downloadVideosFromServer in utils.py")
    print("---------------------------------------------------------------------------------------")
...
def getMetadataFromServer(session_id, justCheckerParams=False):
    ...
    if session['meta'] is not None:
        ...
        print(session['meta']['checkerboard']['placement'])
...
def downloadAndSwitchCalibrationFromDjango(session_id, session_path, calibTrialID=None,
    ...
    if 'meta' in trial.keys() and trial['meta'] is not None and 'calibration' in trial['meta'].keys() and trial['meta'][
        'calibration']:
...
def changeSessionMetadata(session_ids, newMetaDict):
    ...
    if 'datasharing' in newMetaDict:
        if newMetaDict['datasharing'] not in ['Share processed data and identified videos',
                                              ...
                                              'Share no data']:
            raise Exception(
                'datasharing is {} but should be one of the following options: "Share processed data and identified videos", "Share processed data and de-identified videos", "Share processed data", "Share no data".'.format(
                    newMetaDict['datasharing']))
        ...
        if 'filterfrequency' in newMetaDict:
            if newMetaDict['filterfrequency'] != 'default':
                if float(newMetaDict['filterfrequency']) > framerate / 2:
                    raise Exception(
                        'Filter frequency cannot exceed Nyquist frequency (here {}Hz).'.format(framerate / 2))
        ...
        if poseDetector.lower() == 'openpose':
            if 'resolutionPoseDetection' in main_settings:
                ...
                if usedPoseDetector.lower() == poseDetector.lower() and usedResolution == resolutionPoseDetection:
                    print(
                        'The pose pickles for {} {} already exist in the database. We will download them to avoid re-running pose estimation'.format(
                            poseDetector, resolutionPoseDetection))
                    getPosePickles(trial_id, session_path, poseDetector=poseDetector,
                                   resolutionPoseDetection=resolutionPoseDetection)
                else:
                    print(
                        'The pose pickles in the database are for {} {}, but you are now using {} {}. We will re-run pose estimation'.format(
                            usedPoseDetector, usedResolution, poseDetector, resolutionPoseDetection))
            else:
