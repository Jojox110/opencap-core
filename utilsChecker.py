...
def calcIntrinsics(folderName, CheckerBoardParams=None, filenames=['*.jpg'],
    ...
    for iImage, pathName in enumerate(imageFiles):
        ...
        imageSize = np.reshape(np.asarray(np.shape(image)[0:2]).astype(np.float64),
                               (2, 1))  # This all to be able to copy camera param dictionary
...
def synchronizeVideoKeypoints(keypointList, confidenceList,
    ...
    keypointList, confidenceList = zip(
        *[removeOccludedSide(keys, conf, footMkrs, confidenceThreshold, visualize=False) for keys, conf in
          zip(keypointList, confidenceList)])
    # remove occluded arm markers
    keypointList, confidenceList = zip(
        *[removeOccludedSide(keys, conf, armMkrs, confidenceThreshold, visualize=False) for keys, conf in
          zip(keypointList, confidenceList)])
    ...
    markers4VertVel = [mkrDict['RAnkle'], mkrDict[
        'LAnkle']]  # R&L Ankles and Heels did best. There are some issues though - like when foot marker velocity is aligned with camera ray
    ...
    for (keyRaw, conf) in zip(keypointList, confidenceList):
        ...
        vertVelList.append(
            getVertVelocity(keyRaw_clean_smooth))  # doing it again b/c these settings work well for synchronization
        mkrSpeedList.append(getMarkerSpeed(keyRaw_clean_smooth, markers4VertVel, confidence=conf,
                                           averageVels=False))  # doing it again b/c these settings work well for synchronization
    ...
    nansInOutList = []
    for (keyRaw, conf) in zip(keypointList, confidenceList):
        keyRaw_clean, conf_clean, nans_in_out, conf_sync_clean = clean2Dkeypoints(keyRaw, conf, confidenceThreshold,
                                                                                  nCams=nCams)
    ...
    for iCam, vertVel in enumerate(vertVelList):
        ...
        if iCam > 0:
            ...
            if np.abs(
                    lag) > maxShiftSteps:  # if this fails and we get a lag greater than maxShiftSteps (units=timesteps)
                lag = 0
                print(
    ...
    if visualize2Dkeypoint:
        ...
        for camNum in range(nCams):
            ...
            if len(usedInds) > 0:
                ...
                ax.set_ylim((.9 * np.min(keypointListFilt[camNum][mkr, usedInds, mkrXY]),
...
def trackBoundingBox(videoPath, bbStart, allPeople, allBoxes, dataOut, frameStart=0, frameIncrement=1, visualize=False):
    ...
    while frameNum > -1 and frameNum < nFrames:
        ...
        # Find person closest to tracked bounding box, and fill their keypoint data
        keyBoxes = [box[frameNum] for box in allBoxes]
        iPerson, bboxKey, samePerson = findClosestBox(bbox, keyBoxes,
                                                      imageSize)  # it may be better to find the one closest to bboxKey, but then it wouldn't be leveraging
        ...
        if updateCounter > 20:
            ...
            try:
                ...
                ok = tracker.init(frame,
                                  tuple(bboxKey.astype(int)))  # bbox has to be tuple for legacy trackers, like MOSSE
...
def removeOccludedSide(key2D, confidence, mkrInds, confThresh, visualize=False):
    ...
    for mkr in lMkrs:
        if np.count_nonzero(confidence_out[mkr, :] > 0) > 3:
            lNan = np.intersect1d(lOccluded, np.arange(np.argwhere(confidence_out[mkr, :] > 0)[1],
                                                       np.argwhere(confidence_out[mkr, :] > 0)[
                                                           -1]))  # don't add nans to 0 pad
        else:
    ...
    for mkr in rMkrs:
        if np.count_nonzero(confidence_out[mkr, :] > 0) > 3:
            rNan = np.intersect1d(rOccluded, np.arange(np.argwhere(confidence_out[mkr, :] > 0)[1],
                                                       np.argwhere(confidence_out[mkr, :] > 0)[
                                                           -1]))  # don't add nans to 0 pad
        else:
...
def getMarkerSpeed(key2D, idxMkrs=[0], confidence=None, confThresh=0.2, averageVels=False):
    ...
    # 2d marker speed
    vertVel = np.abs(
        np.linalg.norm(np.diff(key2D, axis=1, n=diffOrder, append=np.tile(key2D[:, [-1], :], (1, diffOrder, 1))),
                       axis=2))
...
def filterKeypointsButterworth(key2D, filtFreq, sampleFreq, order=4):
    ...
    if wn > 1:
        print('You tried to filter ' + str(int(sampleFreq)) + ' Hz signal with cutoff freq of ' + str(
            int(filtFreq)) + '. Will filter at ' + str(int(sampleFreq / 2)) + ' instead.')
        wn = 0.99
    elif wn == 1:
        wn = 0.99
...
def filter3DPointsButterworth(points3D, filtFreq, sampleFreq, order=4):
    ...
    wn = filtFreq / (sampleFreq / 2)
    if wn > 1:
        print('You tried to filter ' + str(sampleFreq) + ' signal with cutoff freq of ' + str(
            filtFreq) + ', which is above the Nyquist Frequency. Will filter at ' + str(sampleFreq / 2) + 'instead.')
        wn = 0.99
    elif wn == 1:
        wn = 0.99
...
def cross_corr(y1, y2, multCorrGaussianStd=None, visualize=False, dataForReproj=None, frameRate=60):
    ...
    if dataForReproj is not None:
        ...
        idxPeaks = np.squeeze(
            np.asarray([np.argwhere(peaks['peak_heights'][i] == corr) for i in range(len(peaks['peak_heights']))]))
        lags = idxPeaks - shift
        ...
        if reprojErrorRatio < 0.6 and not False in reprojSuccess:  # tunable parameter. Usually around 0.25 for overground walking
            ...
            if multCorrGaussianStd is not None:
                print('For {}, used reprojection error minimization to sync.'.format(
...
def cross_corr_multiple_timeseries(Y1, Y2, multCorrGaussianStd=None, dataForReproj=None, visualize=False, frameRate=60):
    ...
    if dataForReproj is not None:
        ...
        idxPeaks = np.squeeze(np.asarray(
            [np.argwhere(peaks['peak_heights'][i] == summedCorr) for i in range(len(peaks['peak_heights']))]))
        lags = idxPeaks - shift
        # look at 3 lags closest to 0
        if np.isscalar(lags):
...
def triangulateMultiview(CameraParamList, points2dUndistorted,
    ...
    if selectCamerasMinReprojError and nCams > 2:
        ...
        for iCombo, camCombo in enumerate(camCombos):
            ...
            reprojError[:, iCombo] = calcReprojectionError(cameraList, pointsInput, points3DList[-1],
                                                           weights=confidence)
    ...
    if ransac and nCams > 2:
        nIter = np.round(np.log(.01) / np.log(
            1 - np.power(.75, 2)))  # log(1 - prob of getting optimal set) / log(1-(n_inliers/n_points)^minPtsForModel)
        ...
        for iIter in range(int(nIter)):
            ...
            for iMkr in range(nMkrs):
                ...
                if (len(maybeInliers) + len(alsoInliers[iMkr])) >= nGoodModel:
                    ...
                    point3D = triangulateLimitedCameras(cameraList, [pointsInput[iMkr]], thisConf,
                                                        maybeInliers + alsoInliers[iMkr])
                    er3D = reprojErrorLimitedCameras(cameraList, [pointsInput[iMkr]], point3D, thisConf,
                                                     maybeInliers + alsoInliers[iMkr])
...
def triangulateMultiviewVideo(CameraParamDict, keypointDict, imageScaleFactor=1,
    ...
    for iFrame, points2d in enumerate(unpackedKeypoints):
        ...
        points3D[:, :, iFrame], confidence3D[:, :, iFrame] = triangulateMultiview(CameraParamList_selectedCams,
                                                                                  points2d,
                                                                                  imageScaleFactor=1,
    ...
    if CameraDirectories != None and trialName != None:
        ...
        for iCam, camName in enumerate(keypointDict):
            ...
            inputPaths = glob.glob(
                os.path.join(CameraDirectories[camName], outputMediaFolder, trialName, trialID + '*'))
            if len(inputPaths) > 0:
                inputPath = inputPaths[0]
            else:
                inputPaths = glob.glob(
                    os.path.join(CameraDirectories[camName], 'InputMedia*', trialName, trialID + '*'))
                inputPath = inputPaths[0]
...
def calcReprojectionErrorForSync(CamParamList, keypointList, lagVal,
    ...
    for tStep in range(nTimesteps):
        ...
        confForWeights = [c[:, None, tStep].T for c in
                          confListSelected]  # needs transpose for reprojection error function
        confForWeights = [np.nan_to_num(c, nan=0) for c in
                          confForWeights]  # sometimes confidence has nans, don't want to use as weights in this case
        ...
        if np.any(weightedReprojErrors):
            ...
            reprojErrorVec.append(
                1000)  # in cases where no position is confident set to large reproj error. typical values are on the order of  0.1
...
def loadPklVideo(pklPath, videoFullPath, imageBasedTracker=False, poseDetector='OpenPose',
    ...
    if len(allPeople) > 1:
        ...
        bbFromKeypoints = [keypointsToBoundingBox(data, confidenceThreshold=confidenceThresholdForBB) for data in
                           allPeople]
        maxArea, maxIdx = zip(*[getLargestBoundingBox(data, bbox) for data, bbox in zip(allPeople,
                                                                                        bbFromKeypoints)])  # may want to find largest bounding box size in future instead of height
