    ...
    for session_id in session_ids:
        ...
        if dynamic_ids == None:
            ...
            session = requests.get(API_URL + "sessions/{}/".format(session_id),
                                   headers={"Authorization": "Token {}".format(API_TOKEN)}).json()
            dynamic_ids_toProcess = [t['id'] for t in session['trials'] if
                                     (t['name'] != 'calibration' and t['name'] != 'neutral')]
        else:
...
def runTestSession(pose='all', isDocker=True):
    ...
    if not any(s in API_URL for s in ['dev.opencap', '127.0']):  # prod trials
        trials['hrnet'] = "4108f069-9029-4795-9411-ee3e10f5de83"
    ...
    try:
        ...
        for trial_id in trialList:
            trial = getTrialJson("4108f069-9029-4795-9411-ee3e10f5de83")
        ...
        raise Exception('Failed status check. Stopped. ' + str(e))
